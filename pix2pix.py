...
def load(example_proto):
  ...
  # real_image = tf.io.decode_image(example["real_image"])t
  # input_image = tf.io.decode_image(example["input_image"])
  #image_pair = tf.io.decode_raw(image_file, tf.float32)
  real_image = tf.reshape(real_image, (example["width"], example["height"], 3))
  input_image = tf.reshape(input_image, (IMG_WIDTH, IMG_HEIGHT, 3))
...
def random_crop(input_image, real_image):
  stacked_image = tf.stack([input_image, real_image], axis=2)
...
def load_image_test(image_file):
  ...
  # Create input
#train_dataset = tf.data.Dataset.list_files("output_edges")
train_dataset = tf.data.TFRecordDataset("output_edges/edges.tfrecords")
feature_description = {
...
#   axs[0].imshow(ex[0][:, :, 0] * 0.5 + 0.5, cmap='gray')
#   axs[1].imshow(ex[1]* 0.5 + 0.5)
#   print(ex[1])
...
test_dataset = tf.data.TFRecordDataset("output_edges/test_edges.tfrecords")
